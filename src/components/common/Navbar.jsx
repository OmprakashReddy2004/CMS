...
const Navbar = () => {
  ...
  return (
    <div
      className={`flex h-12 items-center justify-center border-b-[1px] border-b-richblack-700 ${
        location.pathname !== "/" ? "bg-richblack-800" : ""
      } transition-all duration-200 fixed top-0 left-0 right-0 z-50 bg-black`}
    >
      <div className="flex w-11/12 max-w-maxContent items-center justify-between">
        <Link to="/">
          {/* <img src={logo} width={160} height={30} loading='lazy' margin='10px' /> */}
          <img
            src={logo}
            alt="logo"
            width={60}
        ...
        <nav className="max-md:visible ">
          <ul
            className="flex gap-x-6 text-richblack-25 mobile-nav w-[90%]  "
            id="mobileNav"
          >
          ...
          <ul
            className="flex gap-x-6 text-richblack-25 max-md:hidden  "
            id="mobileNav"
          >
            {NavbarLinks.map((link, index) => (
              <li key={index}>
        ...
        <button className="mr-4 text-2xl md:hidden" onClick={showHide}>
          ...
          <AiOutlineClose
            ...
            fill="#AFB2BF"
            id="crossBtn"
            className="hideMenuBtn text-3xl"
          />
        </button>
      </div>
    </div>
